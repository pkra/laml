...
// Markdown
const cmark = document.createElement('script');
cmark.src = 'https://cdnjs.cloudflare.com/ajax/libs/commonmark/0.28.1/commonmark.js';
document.head.appendChild(cmark);

// TODO THIS IS A RACE CONDITION
cmark.onload = function() {
  var reader = new commonmark.Parser();
var writer = new commonmark.HtmlRenderer();
var parsed = reader.parse(document.body.innerHTML); // parsed is a 'Node' tree
...
for (let ref of refs) {
  ...
  targetHeading = renamedNode.innerHTML = target.querySelector(
    'h1, h2, h3, h4, h5, h6'
  ).cloneNode(true);
  // strip blame
  if (targetHeading.querySelector('.blame')) targetHeading.removeChild(targetHeading.querySelector('.blame'));
...
for (let [index, bibitem] of bibitems.entries()) {
  ...
  const cites = document.querySelectorAll('cite[target="' + bibitem.id + '"]');
...
mj.src = 'https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.2/MathJax.js?config=TeX-AMS_CHTML-full';
